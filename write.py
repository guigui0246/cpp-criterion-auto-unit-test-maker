...
def make_test_file(name: str) -> None:
...
def write_test(name: str, fonctions: list[str]):
    name = name.removesuffix(".hpp").removesuffix(".cpp")
    file = os.path.join(os.path.dirname(name), "tests/test_"+os.path.basename(name)+".cpp")
    make_test_file(name)
    with open(file, "a") as f:
        try:
            f.write(f"""\nTest({os.path.dirname(name)}, {
                fonctions[0].split()[1].split("::")[-1].removeprefix("&").removeprefix("*")})\n""")
            f.write("{\n")
